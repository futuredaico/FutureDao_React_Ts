...
export interface IMolochManagerStore {
  ...
  sponsorProposal: (proposalIndex: string, myaddr: string, assetHash: string, depositNum: string, sendCall: (txid: string) => void) => Promise<boolean>,
  ...
  processKickAndProposal: (proposalIndex: string, kickAddress: string, myaddr: string, sendCall: (txid: string) => void) => Promise<boolean>,
