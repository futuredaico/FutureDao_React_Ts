...
class IMolochManager
{
  ...
  @action public getMolochProposalDetail = async (projId: string) =>
  {
    ...
    if(this.proposalInfo){
      const nstr = 'The first DAO proposal submitted had an incorrect amount of shares requested (40 Shares instead of 50 shares)\n\nhttps://docs.google.com/document/d/1fTj_DS0_nQgpuoYqZlv9G-8e4PF8ZzFZtgmb-BTbUkE/edit\n'
      const nstr2 = nstr.replace(/\r?\n/g,"<br />")
      console.log(nstr2)
      const str = this.proposalInfo.proposalDetail.replace(/\n/g,"<br />");
      this.proposalInfo.proposalDetail = this.proposalInfo.proposalDetail.replace(/\r?\n/g,"<br />");
      console.log(str)
